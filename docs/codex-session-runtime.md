...
`codex exec --ephemeral` is useful for a bounded helper call because it writes no session files. It remains a headless shell process, never a Session ref or an Agent presence.
