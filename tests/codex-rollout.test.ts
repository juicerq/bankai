...
import { codexRollouts, IDLE_ROLLOUT, rolloutMeta, turnAfter } from "@main/agents/harness/codex/codex-rollout";
...
describe("reading a rollout header", () => {
	test("names the session, its directory, and calls a cli thread with no parent a root", () => {
		expect(rolloutMeta(metaRecord({}).trim())).toEqual({ sessionId: SESSION, cwd: CWD, root: true });
	});

	test("calls a thread with a parent a subagent, not a root", () => {
		const meta = rolloutMeta(metaRecord({ parent_thread_id: "019f0000-0000-7000-8000-000000000000" }).trim());

		expect(meta?.root).toBe(false);
	});

	test("calls a thread from any other source a non-root", () => {
		expect(rolloutMeta(metaRecord({ source: "exec" }).trim())?.root).toBe(false);
		expect(rolloutMeta(metaRecord({ source: "vscode" }).trim())?.root).toBe(false);
	});

	test("refuses a first record that is not a rollout header", () => {
		expect(rolloutMeta('{"type":"event_msg","payload":{"type":"task_started"}}')).toBeNull();
		expect(rolloutMeta("not json at all")).toBeNull();
		expect(rolloutMeta(JSON.stringify({ type: "session_meta", payload: { cwd: CWD } }))).toBeNull();
	});
});

describe("following the turn edges", () => {
	test("a started task opens a turn dated by the record, not by the clock", () => {
		expect(turnAfter(IDLE_ROLLOUT, [TURN_STARTED])).toEqual({
			turn: { turnId: "turn-1", startedAt: OPENED_AT * 1000 },
		});
	});

	test("a completed task closes the turn and dates the end", () => {
		const state = turnAfter(IDLE_ROLLOUT, [TURN_STARTED, TURN_COMPLETE]);
	...
	test("an aborted turn closes it too", () => {
		const state = turnAfter(IDLE_ROLLOUT, [
			TURN_STARTED,
	...
	test("an unknown event leaves the turn exactly as it was", () => {
		const opened = turnAfter(IDLE_ROLLOUT, [TURN_STARTED]);

		expect(turnAfter(opened, [eventRecord({ type: "agent_reasoning_delta" }), "{ malformed"])).toEqual(opened);
	});

	test("a second start replaces the turn it follows", () => {
		const state = turnAfter(turnAfter(IDLE_ROLLOUT, [TURN_STARTED]), [
...
describe("following a rollout as it grows", () => {
	...
	test("caches a header and forgets it once the rollout is no longer open", async () => {
		...
		expect((await codexRollouts.meta(path))?.sessionId).toBe(SESSION);
		...
		expect(await codexRollouts.meta(path)).toBeNull();
	});
...
describe("a process holding several rollouts", () => {
	test("names the one root among its subagents", async () => {
		...
		const metas = await Promise.all([root, child].map((path) => codexRollouts.meta(path)));
	...
	test("leaves two roots unnamed rather than guessing between them", async () => {
		...
		const metas = await Promise.all([first, second].map((path) => codexRollouts.meta(path)));
