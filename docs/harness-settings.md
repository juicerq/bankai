...
`settings.json` v6 carries `harness: { autostart, id, profiles? }`, one profile per harness holding `args?`, `hooks?` and `naming?` — `hooks` is documented in `hook-spool.md` and `naming` in `session-naming.md`. It is absent until the panel is used; `autostartCommandLine` in `src/main/terminal/autostart.ts` falls back to `DEFAULT_HARNESS_SETTINGS` — autostart on, Claude — so a fresh install and every store written before v3 behave the same.
