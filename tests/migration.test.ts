...
describe("store envelope migration", () => {
	...
	it("throws when the file version is newer than the code version", () => {
		...
		expect(() => store.read()).toThrow(/newer than code version/);
	});

	it("throws when a required migrator is missing", () => {
		...
		expect(() => store.read()).toThrow(/missing migrator from v1/);
	});
