...
Installing it needs `pacman -Syu`, not `-Sy`: the `archlinux:base-devel` image ships a glibc older than the current repositories, and a package pulled in without upgrading the system links against the newer one. `magick` then dies with `libm.so.6: version GLIBC_2.44 not found` inside `package()`, which reads like a broken recipe and is really a partial upgrade.
